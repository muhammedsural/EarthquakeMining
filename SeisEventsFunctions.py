...
def FDSN_EventsToDataFrame(Events : Catalog)->pd.DataFrame:
    ...
    Args:
        Events (Catalog): Events catalog from FDSN Server
...
def GetAfadEvents(StartTime : UTCDateTime = None, EndTime: UTCDateTime = None,FormatType : str = 'JSON')-> list:
    """Brings the earthquakes that occurred in the last two days from AFAD service
...
def GetAfadEvents2(Geom : object  = None, Depth : DepthFilter = None, StartTime : UTCDateTime = None, EndTime: UTCDateTime = None, Magnitude : MagnitudeFilter = None, FormatType : str = 'JSON', OrderBy : str = "timedesc")-> Any:
    """It retrieves data using filtering operations in the AFAD service. Max 5 days of data from the AFAD web service.
    Args:
...
def AfadEventsToDataFrame(afad_geojson : list)->pd.DataFrame:
    """Converts the given afad geojson file to dataframe

    Args:
        afad_geojson (list): afad events datas geojson file

    Returns:
        pd.DataFrame: _description_
    """
    
    feature_list = ['EventId','OriginTime', 'Lat', 'Lon', 'depth','mag', 'magnitude_type']
    ...
    for ii in range (0, len(afad_geojson[0]['features'])):
        df['EventId'].loc[ii]               = afad_geojson[0]['features'][ii]["properties"]['EventID']    
...
def GetAfadEventsByEventId(EventId : int,FormatType : str = 'JSON') -> Any:
    ...
    Args:
        ...
        FormatType (str, optional): _description_. Defaults to 'JSON'. Other options XML,CSV,KML,GEOJSON
